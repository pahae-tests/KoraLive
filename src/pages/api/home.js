...
export default async function handler(req, res) {
  try {
    ...
    await page.goto("https://www.yalla1shoot.com/matches-today_2", {
      waitUntil: "networkidle2",
    });

    await page.waitForSelector("#ayala-today > div");
    ...
    const matches = await page.evaluate(() => {
      ...
      matchDivs.forEach((el, i) => {
        const team1Logo = el.querySelector(
          ".AY_Inner .MT_Team.TM1 .TM_Logo img"
        )?.getAttribute("data-src") || "";
        const team1Name =
          el.querySelector(".AY_Inner .MT_Team.TM1 .TM_Name")?.textContent.trim() || "";

        const team2Logo = el.querySelector(
          ".AY_Inner .MT_Team.TM2 .TM_Logo img"
        )?.getAttribute("data-src") || "";
        const team2Name =
        ...
        const status =
          el.querySelector(".AY_Inner .MT_Data .MT_Stat")?.textContent.trim() || "upcoming";
        const league =
          el.querySelector(".MT_Info ul li:nth-child(3) span")?.textContent.split(",")[1].trim() ||
          "";
